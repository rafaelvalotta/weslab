...
def file2graph_topfarm(filename, input_x,input_y, boundary, rotation=None):
    '''filename is a Matlab .mat file or an Excel
    spreadsheet in the proper format'''
    fpath = Path(filename)
    data = {}
    ...
    WTcoords = np.array([input_x,input_y])

    WTcentroid = np.array([WTcoords[0].mean(), WTcoords[1].mean()])

    #### concatenate substation coordinates (or centroid)    

    ####
    OSScoords, OSSlabels = data['OSS coordinates'] #replace with centroid
    OSScoords = WTcentroid.reshape(2,1)
    boundary = data['WF area limits'][0].T
