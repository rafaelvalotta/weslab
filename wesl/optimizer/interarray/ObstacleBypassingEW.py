...
def OBEW(G_base, capacity=8, weightfun=plain_length,
         maxiter=10000, debug=False, MARGIN=1e-4,
         maxDepth=4, rootlust=None, warnwhere=None):
    ...
    options = dict(weightfun=weightfun.__name__,
                   MARGIN=MARGIN, variant='C',
                   rootlust=rootlust)
    ...
    # Dmax = round(2*N/3/capacity)
    Dmax = N

    # mappings from nodes
    # <Subtree>: maps nodes to the set of nodes in their subtree
    Subtree = np.empty((N + Dmax,), dtype=object)
    Subtree[:N] = [{n} for n in range(N)]
    # Subtree = np.array([{n} for n in range(N)])
    ...
    def plan_detour(root, blocked, goal_, u, v, barrierLo,
        ...
        # if blocked_ in (u, v) and goal_ < 0 and detourHop[-2] >= N:
        if False and blocked_ in (u, v) and goal_ < 0 and detourHop[-2] >= N:
                ...
                for j, (hop2check, prevhop) in enumerate(
                    ...
                    if is_concave_at_hop2check:
                        ...
                        if hop2check not in remove:
                            if prevhop >= N and hop2check not in Barrier:
    ...
    def add_corner(hook, corner_, cleatnode, loNotHi):
        ...
        length = np.hypot(*(VertexC[fnT[hook]] -
                            VertexC[corner_]).T)
        G.add_edge(hook, corner, length=length, weight=length,
    ...
    def move_corner(corner, hook, corner_, cleatnode, loNotHi):
        ...
        G[hook][corner].update(length=farL, weight=farL)
    ...
    def make_detour(blocked, path, LoNotHi, shift):
        ...
        if not Corner_:
            ...
            G.add_edge(hook, root, length=length, weight=length,
                       type='detour', color='yellow', style='dashed')
            log.append((i, 'addDE', (hook, root, fnT[hook], root)))
            Final_G[root].add(hook)
        else:
            detourHop = DetourHop[cleatnode]
            if blocked < N or hook == blocked:
                ...
                G.add_edge(corner, root, length=length, weight=length,
                           type='detour', color='yellow', style='dashed')
                ...
                G[corner][root].update(length=nearL, weight=nearL)
        ...
        for root in roots2check:
            for g2check in Final_G[root] - {v}:
                if (is_crossing_gate(root, g2check, u, v,
                        ...
                        (g2check >= N and fnT[g2check] in (u, v) and
    ...
    def loop():
        ...
        while True:
            ...
            if not eX and D:
                ...
                for s, t in G.edges(range(N, N + D)):
                    skip = False
                    if s < 0 or t < 0:
                    ...
                    if (s >= N and (s_ == u or s_ == v) and
                            s != DetourHop[gnT[s]][-1]):
                        Corner.append(s)
                    if (t >= N and (t_ == u or t_ == v) and
                            t != DetourHop[gnT[t]][-1]):
                        Corner.append(t)
